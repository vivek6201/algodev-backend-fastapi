...
async def validation_exception_handler(
    ...
    if isinstance(exc, RequestValidationError):
        ...
        for error in errors:
            ...
            error = {"field": str(field), "message": msg}
            error_details.append(error)
        return ErrorResponse(
            message="Validation error", error=error_details, status_code=status.HTTP_400_BAD_REQUEST
        )

    return ErrorResponse(
        message="Validation error", error=str(exc), status_code=status.HTTP_400_BAD_REQUEST
    )
