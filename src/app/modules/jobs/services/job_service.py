from app.modules.jobs.schemas.job_validations import ThirdPartyJobCreate, DirectJobCreate, ThirdPartyJobUpdate
from app.modules.jobs.schemas.category_validations import CategoryCreate, CategoryUpdate
from sqlmodel import Session, select
from app.modules.jobs.models.jobs import Job, JobStatus, Category, ListingType
from typing import Optional
from app.common.lib.formatter import TokenPayload


class JobService:
    def getJob(self, session: Session, job_id: Optional[int], slug: Optional[str] = None, listing_type: Optional[ListingType] = None, user_role: Optional[str] = None):
        job = None

        if slug:
            query = select(Job).where(Job.slug == slug)
            if listing_type:
        ...
        if job_id:
            ...
            if job and listing_type and job.listing_type != listing_type:
                return None
            # Filter by status based on user role
            if job and user_role not in ["ADMIN", "MODERATOR", "RECRUITER"] and job.status != JobStatus.PUBLISHED:
                return None
    ...
    def listJobs(self, session: Session, listing_type: Optional[ListingType] = None, user_role: Optional[str] = None):
