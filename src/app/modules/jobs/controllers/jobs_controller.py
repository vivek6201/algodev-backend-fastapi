...
class JobsController:
    ...
    def create_third_party_job(
        ...
        return SuccessResponse(
            data=job_response, message="Job created successfully", status_code=201
        )

    def update_third_party_job(
        self, session: Session, user: TokenPayload, job_id: int, job_data: ThirdPartyJobUpdate
    ):
        ...
        job_response = JobResponse.model_validate(updated_job)
        return SuccessResponse(data=job_response, message="Job updated successfully")
    ...
    def list_jobs(
        ...
        return SuccessResponse(data=job_responses, message="Jobs retrieved successfully")
    ...
    def get_job(
        ...
        user_role = current_user.role if current_user else None
        job = self.job_service.getJob(
            session, job_id, listing_type=listing_type, user_role=user_role
        )
        if not job:
            return ErrorResponse(message="Job not found", status_code=404)
        job_response = JobResponse.model_validate(job)
        return SuccessResponse(data=job_response, message="Job retrieved successfully")
    ...
    def create_category(self, session: Session, category_data: CategoryCreate):
        ...
        return SuccessResponse(
            data=new_category, message="Category created successfully", status_code=201
        )
